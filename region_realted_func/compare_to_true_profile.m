function [gene_inds_true_type,gene_inds_predictions, ...
        ...
        compare_to_true_profile(true_cell_types,gene_info, species,parms,true_cell_species)
% This function matches the gene names between the data in question and the
% true-celltype data
% It returns the gene which appear in both datasets.
% It return the indices of these genes and the symbols
   

    if ~exist('true_cell_species','var')
        true_cell_species = 'mouse';
    end
    ...
    switch species
        case 'mouse'
            ...
            [reorder_true_cell_type, reorder_predicted] = ...
                reorderUsingId(true_cell_types.all_symbols, ...
                               gene_info.gene_symbols);
            reorder_true_cell_type = ...
                true_cell_types.refer_to_index(reorder_true_cell_type);
            % mouse_cell_types.expression =
            % mouse_cell_types.expression(reorder_true_cell_type,:);
            % predicted_profiles = cellfun(@(x)
            % x(reorder_predicted,:),
            % predicted_profiles,'UniformOutput',false);
            gene_inds_true_type = reorder_true_cell_type;
            gene_inds_predictions = reorder_predicted;

            gene_symb_true_type  = true_cell_types.gene_symbol(reorder_true_cell_type);
            gene_symb_predictions  = gene_info.gene_symbols(reorder_predicted);

      case {'monkey', 'human'}
          switch true_cell_species
              case 'human'
                [reorder_true_cell_type, reorder_predicted] = ...
                    reorderUsingId(true_cell_types.all_symbols, ...
                               gene_info.gene_symbols);
              ...
              case 'mouse'
                ...
                gene_symb_true_type  = true_cell_types.gene_symbol(gene_to_group_mouse);
                gene_symb_predictions  = gene_info.gene_symbols(gene_to_group_primate);
              otherwise
                  error('unkown true species');
          end
...
%     true_profiles = match_region_with_true_profile(mouse_cell_types, region_name,parms);
...
function [expressionA,expressionB] = limit_to_genes_in_intersection(expressionA,expressionB, gene_list_A, gene_list_B)
    ...
    reorder_true_cell_type = mouse_cell_types.refer_to_index(reorder_A);

    expressionA = expressionA(reorder_true_cell_type,:);
