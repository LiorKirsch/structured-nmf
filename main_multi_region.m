...
[num_genes, num_samples] = size(mix_data.expression);
rand_subset = createRandSubset(num_samples, num_samples_list, ...
    parms.subsample_repeats, mix_data.sample_group); %TODO=>region

X = mix_data.expression;
GT_profiles = mix_data.profiles';
GT_proportions = mix_data.proportions';
sample_group = mix_data.sample_group;
profile_group = mix_data.profile_group;
...
[scores, proportions_scores] = loopOverHyperparms_multi(X, GT_profiles, ...
    GT_proportions, sample_group, profile_group, rand_subset, parms, ...
