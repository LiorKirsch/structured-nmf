function draw_profiles_with_GT(profiles,proportions,GT_profile,parms)

region_name = parms.relation_regions;
[ k, num_genes] = size(profiles{1});

all_profiles = cat(1,profiles{:});
all_GT_profiles = cat(1,GT_profile{:});

region_name = repmat(region_name,1,k)';
region_name = region_name(:);

[wcoeff,score,latent,tsquared,explained] = pca(all_GT_profiles);

score = bsxfun(@minus, all_profiles, mean(all_profiles,1)) * wcoeff;
