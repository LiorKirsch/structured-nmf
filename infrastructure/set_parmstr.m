...
%
    ...
    if isfield(parms, 'restart_ind')
        parmstr = sprintf('%s_RRInd%d', parmstr, parms.restart_ind);
    end
    ...
    if isfield(parms, 'random_init_spread')
        if parms.random_init_spread ~= 1
            parmstr = sprintf('%s_RIS%g', parmstr, parms.random_init_spread);
        end
    end
    
    %parmstr2 = set_parmstr2(parms);
    %if ~ strcmp(parmstr, parmstr2)
    %    error('parmstr mismatch\n[%s]\n[%s]', parmstr, parmstr2);
    %end
