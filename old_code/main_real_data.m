...
parms = conf(parms);
parms.maxiter = 3000;
parms.W_constraints = 'positive'; %'on_simplex';
% parms.nmf_method = 'alsAccProj';
parms.nmf_method = 'cjlin';


dataset = take_from_struct(parms, 'dataset', 'brainspan2014');
[default_regions, parms.species] = get_region_set(dataset);
regions_ack = take_from_struct(parms, 'regions_ack', default_regions);
regions_ack = sort(regions_ack);

% Load all data
[parms.dataset_file, expression, gross_region_vec, gene_info, ...
 gross_structures_info] = load_all(dataset, regions_ack);
...
results = loopOverHyperparms_new(X, gene_info, parms, loop_over_var_name, loop_over_var_value ,'rand_run');
