function [results, H_results, W_results] = loopOverHyperparms_real(X, gene_info,...
...
%    results.mean_score - the average score (across all regions) - 1 score.
  ...
  if length(loop_over_var_value) ==1
     ...
     H_results = cell(length(var_values),1);
     W_results = cell(length(var_values),1);
...
% % % %                                         parms.num_types,parms);
        ...
        [parms.gene_hash, region_names, curr_gene_info,curr_X, gene_inds_true_type,...
...
%          current_parms = get_current_markers(current_parms, curr_gene_info.gene_symbols);
      ...
      for i_vars = 1: length(var_values)
          ...
          [new_loop_string,current_parms] = ...
              add_var_to_loop_string(loop_string, var_values, i_vars, ...
                                     var_name, current_parms);
          [results{i_vars},H_results{i_vars}, W_results{i_vars}] = ...
              loopOverHyperparms_real(X, gene_info, ...
                                        current_parms, ...
                                        loop_over_var_name, ...
                                        loop_over_var_value, ...
                                        new_loop_string);
